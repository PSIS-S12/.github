...
ray.init(num_gpus=1, memory=8*1024*1024*1024, object_store_memory=2*1024*1024*1024)
...
def writeEpisodeRatio(global_summary, numIL, numRL, sess, curr_episode):
    ...
    RL_IL_Ratio = numRL / (numRL + numIL) if (numRL + numIL) > 0 else 0
    ...
    with tf.Session(config=config) as sess:
        ...
        try:
            while True:
                ...
                # # Stop after first RL episode completes (for evaluation)
                # if not info['is_imitation'] and jobResults:
                #     print("Got one RL episode, stopping.")
                #     break
