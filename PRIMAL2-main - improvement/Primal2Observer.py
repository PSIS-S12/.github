...
class Primal2Observer(ObservationBuilder):
    ...
    def __init__(self, observation_size=11, num_future_steps=3, printTime=False):
        ...
        self.NUM_CHANNELS = 9 + self.num_future_steps  # +1 for global congestion heatmap
        self.printTime = printTime

    def set_world(self, world):
        super().set_env(world)

    def _compute_global_heatmap(self, agent_id):
        """
        Computes a global congestion heatmap showing all OTHER agent positions
        across the entire world, downsampled to observation_size x observation_size.
        Values are in [0, 1] where 1 means at least one agent is present.
        """
        ...
        # downsample to observation_size x observation_size
        scale_h = self.observation_size / H
        scale_w = self.observation_size / W
        heatmap_small = zoom(heatmap, (scale_h, scale_w), order=1)
        heatmap_small = np.clip(heatmap_small, 0.0, 1.0)
    ...
    def _get(self, agent_id, all_astar_maps):
        ...
        # global congestion heatmap (channel 8+num_future_steps)
        heatmap = self._compute_global_heatmap(agent_id)
        state = np.concatenate((state, heatmap[np.newaxis, :, :]), axis=0)
