...
class Runner(object):
    ...
    def __init__(self, metaAgentID):
        ...
        if self.metaAgentID < NUM_IL_META_AGENTS:
            ...
            # force CPU to avoid cuBLAS crashes with RTX 4090 + TF1.15
            config = tf.ConfigProto(allow_soft_placement=True, device_count={"GPU": 0})
    ...
    def multiThreadedJob(self, episodeNumber):
        ...
        if not is_imitation:
            ...
            perf_metrics = np.array([p for p in perf_metrics if p is not None and len(p) == 6])
            if len(perf_metrics) == 0:
                perf_metrics = np.zeros((1, 6))
