...
Policy::Policy(const Graph &g, const RobCoord goal){
  ...
  auto index_map = boost::get(boost::vertex_index, g_);
  auto dist_map = boost::make_iterator_property_map(costs_.begin(), index_map);
  auto pred_map = boost::make_iterator_property_map(predecessors_.begin(), index_map);

  boost::dijkstra_shortest_paths(
    boost::make_reverse_graph(g_), goal,
    boost::predecessor_map(pred_map).distance_map(dist_map));
